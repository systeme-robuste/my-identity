...
## Schema map (28 tables)

| Domain | Tables | Notes |
|--------|--------|-------|
| Identity & access | `users`, `sessions`, `api_keys`, `oauth_*` (3 tables) | Argon2id-style password (PBKDF2/210K), OAuth code + token storage |
| Sites & content | `sites`, `pages`, `site_members`, `cms_collections`, `cms_entries`, `media` | One `sites` row per customer site, multi-page |
| Forms & engagement | `forms`, `form_submissions` | Server-side validation, anti-spam |
| Email & broadcast | `subscribers`, `email_templates`, `email_broadcasts` | List per site, 5 templates on Pro |
| Commerce | `products`, `orders`, `order_items` | Stripe-backed; full order ledger |
| Memberships | `members`, `membership_tiers` | 1 tier on Pro, 5 on Business |
| Automation & analytics | `automations`, `automation_logs`, `analytics_events`, `usage_events` | Event-sourced analytics; usage metered for billing |
| Webhooks & ops | `webhooks`, `webhook_deliveries`, `audit_log`, `abuse_reports`, `data_export_requests` | Outgoing + incoming webhooks; RGPD export/delete; abuse signals |

## Conventions

- All timestamps are `INTEGER` (Unix epoch ms, UTC). Portable Postgres + D1. Generated by the app via `Date.now()`.
- All IDs are ULID strings (26 chars, lexicographically sortable) generated via `ulid()` from `@my-identity/shared/utils/id`.
- All `*_at` columns are non-nullable.
- JSON data is stored as `TEXT` (JSON-encoded string) — keeps schema portable.
- Soft delete is **not** used. Hard delete + `audit_log` row.
